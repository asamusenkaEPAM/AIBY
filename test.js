...
function resultFor(value) {
  if (value >= 9) return { level: "AI-native", title: "Вы управляете системой, а не только промптом", copy: "Вы уверенно различаете чат и агента, понимаете контекст, инструменты и проверку результата. Следующий шаг — создавать собственные многоэтапные процессы и измерять их качество." };
  if (value >= 7) return { level: "Agent-ready", title: "База для агентной работы уже есть", copy: "Вы понимаете основные компоненты современной AI-системы. Восполните обнаруженные пробелы и закрепите знания на практике в небольшом проекте." };
  if (value >= 4) return { level: "AI-практик", title: "Пора выйти за пределы чата", copy: "Вы уже умеете использовать AI с пользой, но инженерная сторона работы пока остаётся непривычной. Начните с одного агента, одного инструмента и измеримого критерия успеха." };
