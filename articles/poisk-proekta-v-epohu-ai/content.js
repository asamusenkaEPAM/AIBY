...
const quizQuestions = [
  ...
  },
  {
    ...
    answers: [
      ...
      "Модель меняет код небольшими порциями, но принимает каждую из них по собственной оценке уверенности"
    ],
    correct: 2,
  ...
  },
  {
    ...
    answers: [
      "Хранит документы и embeddings в едином формате, чтобы заменить отдельную векторную базу данных",
  ...
  },
  {
    ...
    answers: [
      "Чтобы хранить пароли, API-ключи и другие секреты рядом с embeddings — так агент их точно не потеряет",
      "Чтобы находить близкие по смыслу фрагменты, даже когда запрос и документ используют разные слова",
      "Чтобы заменить реляционные базы: векторы одинаково хорошо обслуживают транзакции, связи и отчёты",
  ...
  },
  {
    ...
    answers: [
      ...
      "Перед действием с высокой ценой ошибки: платежом, публикацией, удалением или изменением production"
    ],
    correct: 3,
  ...
  },
  {
    ...
    answers: [
      "Трассу шагов, вызовы инструментов, входные и выходные данные, ошибки, задержки и стоимость выполнения",
      "Финальный ответ и пользовательскую оценку: промежуточные действия только усложняют расследование",
      "Общее время и стоимость запуска: если обе метрики в норме, причина ошибки почти всегда в модели",
      "Размер системного промпта, число токенов и название модели — этого достаточно для полной диагностики"
    ],
    correct: 0,
  ...
  },
  {
    ...
    answers: [
      ...
      "Проверить стиль, названия и форматирование; архитектурные решения лучше оставить модели с большим контекстом",
      "Проверить diff, допущения, тесты, безопасность и влияние изменения на соседние компоненты системы",
